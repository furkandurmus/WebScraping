...
films = [i.h3.a.get_text() for i in movies]
ratings = [i.find('strong').get_text() for i in movies]
metascore = []
for i in movies:
...
release_year = [i.find('span', class_="lister-item-year text-muted unbold").get_text() for i in movies]
votings=[i.find('span', attrs = {'name':'nv'}).text for i in movies]
